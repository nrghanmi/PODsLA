...
for  (i in 1:nrow(area)){
  for (j in 1:nrow(area)){
    if(i!=j){
      sim[i,j] <- mean(c(popNormalized$`area[, 1]`[j],popNormalized$`area[, 1]`[i])) * sim[i,j]
    } 
  }
}
...
setwd("/Results/Excel/")
write_xlsx(as.data.frame(TolPOP),"pop_cov_PW-AP_Avg.xlsx")
...
png("/Results/Figures/PW-AP-ONE-Avg.png", units="in", width=10, height=10, res=150)
