...
#----------------- Normalization -------------------
#----- [-1,1] -----
preprocessParams<-preProcess(area[,3:4], rangeBounds = c(-1,1), method="range")
areaNormalized <- predict(preprocessParams, area[,3:4])
...
setwd("/Results/Excel/")
#-----please select the required state----
write_xlsx(as.data.frame(TolPOP),"pop_cov_S-PODsLA_ONE.xlsx")
#write_xlsx(as.data.frame(TolPOP),"pop_cov_S-PODsLA_TWO.xlsx")
#write_xlsx(as.data.frame(TolPOP),"pop_cov_S-PODsLA_THREE.xlsx")
...
#-----please select the required state----
png("S-PODsLA-ONE.png", units="in", width=10, height=10, res=150)
#png("S-PODsLA-TWO.png", units="in", width=10, height=10, res=150)
#png("S-PODsLA-THREE.png", units="in", width=10, height=12, res=150)
